...
const wake_on_lan_new_template = '<div class="col-sm-4" id="{{ .Id }}">\n' +
    ...
    '\t\t\t<span class="info-box-text"><i class="fas fa-clock"></i> <span class="latest-used">' + __.unused + '</span></span>\n' +
    '\t\t</div>\n' +
    '\t</div>\n' +
    '</div>';
...
jQuery(function ($) {
    let newHostHtml = '<div class="col-sm-2 offset-md-4" style=" text-align: right;"><button style="" id="btn_new_wake_on_lan_host" type="button" class="btn btn-outline-primary btn-sm" data-toggle="modal" data-target="#modal_wake_on_lan_host"><i class="nav-icon fas fa-plus"></i> ' + __.new_host + '</button></div>';
...
jQuery(function ($) {
    ...
    $modal_remove_wake_on_lan_host.on('show.bs.modal', function (event) {
        ...
        $modal.find('.modal-body').text(__.wol_remove_body + $editBtn.data('name'));
...
jQuery(function ($) {
    $('.latest-used').each(function () {
        ...
        try {
            if (timeText != "Unused" && timeText != __.unused) {
...
jQuery(function ($) {
    ...
    let validator = $frm_wake_on_lan_host.validate({
        ...
        messages: {
            name: {
                required: __.wol_validation_name
            },
            mac_address: {
                required: __.wol_validation_mac_required
            }
        },
        errorElement: 'span',
