...
func Favicon() echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		return c.Redirect(http.StatusFound, util.BasePath+"/static/img/favicon.ico")
	}
}
...
func Login(db store.IStore) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		if err != nil {
			return c.JSON(http.StatusBadRequest, jsonHTTPResponse{false, "请求数据错误"})
		}
		...
		if !usernameRegexp.MatchString(username) {
			return c.JSON(http.StatusBadRequest, jsonHTTPResponse{false, "请输入有效的用户名"})
		}

		dbuser, err := db.GetUserByName(username)
		if err != nil {
			log.Infof("Cannot query user %s from DB", username)
			return c.JSON(http.StatusInternalServerError, jsonHTTPResponse{false, "用户名或密码错误"})
		}
		...
		if dbuser.PasswordHash != "" {
			...
			if err != nil {
				return c.JSON(http.StatusInternalServerError, jsonHTTPResponse{false, "无法验证密码"})
			}
		...
		if userCorrect && passwordCorrect {
			...
			return c.JSON(http.StatusOK, jsonHTTPResponse{true, "登录成功"})
		}

		return c.JSON(http.StatusUnauthorized, jsonHTTPResponse{false, "用户名或密码错误"})
	}
}
...
func GetUser(db store.IStore) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		if !isAdmin(c) && (username != currentUser(c)) {
			return c.JSON(http.StatusForbidden, jsonHTTPResponse{false, "操作员无法访问其他用户数据"})
		}

		userData, err := db.GetUserByName(username)
		if err != nil {
			return c.JSON(http.StatusNotFound, jsonHTTPResponse{false, "未找到用户"})
		}
...
func UpdateUser(db store.IStore) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		if username != previousUsername {
			...
			if err == nil {
				return c.JSON(http.StatusBadRequest, jsonHTTPResponse{false, "该用户名已被使用"})
			}
		}
		...
		return c.JSON(http.StatusOK, jsonHTTPResponse{true, "更新用户信息成功"})
	}
}
...
func CreateUser(db store.IStore) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		return c.JSON(http.StatusOK, jsonHTTPResponse{true, "创建用户成功"})
	}
}
...
func RemoveUser(db store.IStore) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		if username == currentUser(c) {
			return c.JSON(http.StatusForbidden, jsonHTTPResponse{false, "用户不能删除自己"})
		}
		// delete user from database

		if err := db.DeleteUser(username); err != nil {
			log.Error("Cannot delete user: ", err)
			return c.JSON(http.StatusInternalServerError, jsonHTTPResponse{false, "无法从数据库删除用户"})
		}

		log.Infof("Removed user: %s", username)

		return c.JSON(http.StatusOK, jsonHTTPResponse{true, "用户已删除"})
	}
}
...
func GetClient(db store.IStore) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		if _, err := xid.FromString(clientID); err != nil {
			return c.JSON(http.StatusBadRequest, jsonHTTPResponse{false, "请输入有效的客户端 ID"})
		}
		...
		if err != nil {
			return c.JSON(http.StatusNotFound, jsonHTTPResponse{false, "未找到客户端"})
		}
...
func NewClient(db store.IStore) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		if client.TgUserid != "" {
			...
			if err != nil || idNum == 0 {
				return c.JSON(http.StatusBadRequest, jsonHTTPResponse{false, "Telegram 用户 ID 必须是非零数字"})
			}
		}
		...
		if util.ValidateAllowedIPs(client.AllowedIPs) == false {
			...
			return c.JSON(http.StatusBadRequest, jsonHTTPResponse{false, "允许的 IP 必须使用 CIDR 格式"})
		}
		...
		if util.ValidateExtraAllowedIPs(client.ExtraAllowedIPs) == false {
			...
			return c.JSON(http.StatusBadRequest, jsonHTTPResponse{false, "额外的允许 IP 必须使用 CIDR 格式"})
		}
		...
		if client.PublicKey == "" {
			...
			if err != nil {
				...
				return c.JSON(http.StatusInternalServerError, jsonHTTPResponse{false, "无法生成 WireGuard 密钥对"})
			}
			...
			if err != nil {
				...
				return c.JSON(http.StatusInternalServerError, jsonHTTPResponse{false, "无法验证 WireGuard 公钥"})
			}
			// check for duplicates
			clients, err := db.GetClients(false)
			if err != nil {
				log.Error("Cannot get clients for duplicate check")
				return c.JSON(http.StatusInternalServerError, jsonHTTPResponse{false, "无法获取客户端列表进行重复检查"})
			}
			for _, other := range clients {
				if other.Client.PublicKey == client.PublicKey {
					log.Error("Duplicate Public Key")
					return c.JSON(http.StatusInternalServerError, jsonHTTPResponse{false, "公钥重复"})
				}
			}
		}
		...
		if client.PresharedKey == "" {
			...
			if err != nil {
				...
				return c.JSON(http.StatusInternalServerError, jsonHTTPResponse{false, "无法验证 WireGuard 预共享密钥"})
			}
		}
		...
		// auto-send email if enabled and client has email
		if client.Email != "" {
			go func() {
				settings, err := db.GetGlobalSettings()
				if err != nil {
					log.Error("Cannot get global settings for auto email: ", err)
					return
				}
				...
				server, err := db.GetServer()
				if err != nil {
					log.Error("Cannot get server config for auto email: ", err)
					return
				}
				config := util.BuildClientConfig(client, server, settings)
				mailer := mailerFromGlobalSetting(settings)
				cfgAtt := emailer.Attachment{Name: "wg0.conf", Data: []byte(config)}
				attachments := []emailer.Attachment{cfgAtt}

				subject := "您的 WireGuard 配置"
				content := "您好,</br><p>附件是您的 WireGuard 客户端配置文件。请将其导入 WireGuard 客户端以连接到服务器。</p><p>谢谢</p>"
				err = mailer.Send(client.Name, client.Email, subject, content, attachments)
				if err != nil {
					log.Errorf("Failed to send auto email to %s: %v", client.Email, err)
				} else {
					log.Infof("Auto email sent to %s for client %s", client.Email, client.Name)
				}
			}()
		}
...
// mailerFromGlobalSetting creates an emailer from global settings
func mailerFromGlobalSetting(s model.GlobalSetting) emailer.Emailer {
	return emailer.NewSmtpMail(
		s.SmtpHostname,
		s.SmtpPort,
		s.SmtpUsername,
		s.SmtpPassword,
...
func EmailClient(db store.IStore, mailer emailer.Emailer, emailSubject, emailContent string) echo.HandlerFunc {
	...
	return func(c echo.Context) error {
		...
		if clientData.Client.PrivateKey != "" {
			...
			if err != nil {
				return c.JSON(http.StatusInternalServerError, jsonHTTPResponse{false, "解码失败: " + err.Error()})
			}
		...
		return c.JSON(http.StatusOK, jsonHTTPResponse{true, "邮件发送成功"})
	}
}
...
func SendTelegramClient(db store.IStore) echo.HandlerFunc {
	...
	return func(c echo.Context) error {
		...
		if clientData.Client.PrivateKey != "" {
			...
			if err != nil {
				return c.JSON(http.StatusInternalServerError, jsonHTTPResponse{false, "二维码生成失败: " + err.Error()})
			}
		}

		userid, err := strconv.ParseInt(clientData.Client.TgUserid, 10, 64)
		if err != nil {
			return c.JSON(http.StatusInternalServerError, jsonHTTPResponse{false, "用户 ID 错误: " + err.Error()})
		}
		...
		return c.JSON(http.StatusOK, jsonHTTPResponse{true, "Telegram 消息发送成功"})
	}
}
...
func UpdateClient(db store.IStore) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		if util.ValidateExtraAllowedIPs(_client.ExtraAllowedIPs) == false {
			...
			return c.JSON(http.StatusBadRequest, jsonHTTPResponse{false, "额外允许的 IP 必须使用 CIDR 格式"})
		}
		...
		if client.PublicKey != _client.PublicKey && _client.PublicKey != "" {
			...
			if err != nil {
				...
				return c.JSON(http.StatusInternalServerError, jsonHTTPResponse{false, "无法验证提供的 WireGuard 公钥"})
			}
			// check for duplicates
			clients, err := db.GetClients(false)
			if err != nil {
				log.Error("Cannot get client list for duplicate public key check")
				return c.JSON(http.StatusInternalServerError, jsonHTTPResponse{false, "无法获取客户端列表进行公钥重复检查"})
			}
			for _, other := range clients {
		...
		if client.PresharedKey != _client.PresharedKey && _client.PresharedKey != "" {
			...
			if err != nil {
				...
				return c.JSON(http.StatusInternalServerError, jsonHTTPResponse{false, "无法验证提供的 WireGuard 预共享密钥"})
			}
		}
		...
		return c.JSON(http.StatusOK, jsonHTTPResponse{true, "更新客户端成功"})
	}
}
...
func SetClientStatus(db store.IStore) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		return c.JSON(http.StatusOK, jsonHTTPResponse{true, "修改客户端状态成功"})
	}
}
...
func DownloadClient(db store.IStore) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		if clientID == "" {
			return c.JSON(http.StatusNotFound, jsonHTTPResponse{false, "缺少 clientid 参数"})
		}
...
func RemoveClient(db store.IStore) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		if err := db.DeleteClient(client.ID); err != nil {
			...
			return c.JSON(http.StatusInternalServerError, jsonHTTPResponse{false, "无法从数据库删除客户端"})
		}

		log.Infof("Removed wireguard client: %v", client)
		return c.JSON(http.StatusOK, jsonHTTPResponse{true, "客户端已删除"})
	}
}
...
func WireGuardServerInterfaces(db store.IStore) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		if util.ValidateServerAddresses(serverInterface.Addresses) == false {
			...
			return c.JSON(http.StatusBadRequest, jsonHTTPResponse{false, "接口 IP 地址必须使用 CIDR 格式"})
		}
		...
		if err := db.SaveServerInterface(serverInterface); err != nil {
			return c.JSON(http.StatusInternalServerError, jsonHTTPResponse{false, "接口 IP 地址必须使用 CIDR 格式"})
		}
		log.Infof("Updated wireguard server interfaces settings: %v", serverInterface)

		return c.JSON(http.StatusOK, jsonHTTPResponse{true, "更新接口地址成功"})
	}
}
...
func GlobalSettingSubmit(db store.IStore) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		if util.ValidateIPAddressList(globalSettings.DNSServers) == false {
			...
			return c.JSON(http.StatusBadRequest, jsonHTTPResponse{false, "无效的 DNS 服务器地址"})
		}
		...
		return c.JSON(http.StatusOK, jsonHTTPResponse{true, "更新全局设置成功"})
	}
}
...
func MachineIPAddresses() echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		if err != nil {
			return c.JSON(http.StatusInternalServerError, jsonHTTPResponse{false, "无法获取机器 IP 地址"})
		}
...
func ApplyServerConfig(db store.IStore, tmplDir fs.FS) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		if err != nil {
			log.Error("Cannot get server config: ", err)
			return c.JSON(http.StatusInternalServerError, jsonHTTPResponse{false, "无法获取服务端配置"})
		}

		clients, err := db.GetClients(false)
		if err != nil {
			log.Error("Cannot get client config: ", err)
			return c.JSON(http.StatusInternalServerError, jsonHTTPResponse{false, "无法获取客户端配置"})
		}

		users, err := db.GetUsers()
		if err != nil {
			log.Error("Cannot get users config: ", err)
			return c.JSON(http.StatusInternalServerError, jsonHTTPResponse{false, "无法获取用户配置"})
		}

		settings, err := db.GetGlobalSettings()
		if err != nil {
			log.Error("Cannot get global settings: ", err)
			return c.JSON(http.StatusInternalServerError, jsonHTTPResponse{false, "无法获取全局设置"})
		}
		...
		return c.JSON(http.StatusOK, jsonHTTPResponse{true, "应用服务端配置成功"})
	}
}
...
func GetHashesChanges(db store.IStore) echo.HandlerFunc {
	return func(c echo.Context) error {
		if util.HashesChanged(db) {
			return c.JSON(http.StatusOK, jsonHTTPResponse{true, "配置哈希已变化"})
		} else {
			return c.JSON(http.StatusOK, jsonHTTPResponse{false, "配置哈希未变化"})
		}
	}
}

// SetLang handler to switch language
func SetLang() echo.HandlerFunc {
	return func(c echo.Context) error {
