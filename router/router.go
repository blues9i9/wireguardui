...
import (
	...
	"github.com/ngoduykhanh/wireguard-ui/i18n"
...
func (t *TemplateRegistry) Render(w io.Writer, name string, data interface{}, c echo.Context) error {
	...
	lang := "zh"
	if ck, err := c.Cookie("lang"); err == nil {
		v := ck.Value
		if v == "en" || v == "zh" {
			lang = v
		}
	...
	// inject more app data information. E.g. appVersion, Messages, Lang
	if reflect.TypeOf(data).Kind() == reflect.Map {
		...
		data.(map[string]interface{})["Messages"] = i18n.GetLocale(lang).Messages
...
func New(tmplDir fs.FS, extraData map[string]interface{}, secret [64]byte) *echo.Echo {
	...
	funcs := template.FuncMap{
		...
		"t": func(key string, m interface{}) string {
			switch ctx := m.(type) {
			...
			case map[string]interface{}:
				...
				// also check Messages sub-key
				if msgs, ok := ctx["Messages"]; ok {
					if mm, ok2 := msgs.(map[string]string); ok2 {
						if v, ok3 := mm[key]; ok3 {
							return v
						}
					}
