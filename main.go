...
func main() {
	...
	app.GET(util.BasePath+"/set-lang", handler.SetLang())
	app.GET(util.BasePath+"/about", handler.AboutPage())
	app.GET(util.BasePath+"/api/wireguard-status", handler.WireGuardStatus(db), handler.ValidSession, handler.NeedsAdmin)
	app.POST(util.BasePath+"/api/install-wireguard", handler.InstallWireGuard(db), handler.ValidSession, handler.ContentTypeJson, handler.NeedsAdmin)
